...
**Real resource cost, and why it stays stopped by default**

This is four containers (Cassandra, Elasticsearch, Cortex, TheHive) on top of the three Wazuh containers already running, on a 16GB Mac also running LocalStack, Cowrie, and sometimes a Windows VM under QEMU. None of them originally had a Docker-level memory limit — only Cassandra and Elasticsearch had their own JVM heap capped (512M each) via environment variables, while Cortex and TheHive's JVMs had no cap at all and could claim as much as the host allowed under memory pressure. Fixed with explicit `mem_limit` on all four services in `docker-compose.yml` (Cassandra 1g, Elasticsearch 1g, Cortex 768m, TheHive 1g — a documented ~3.75GB combined ceiling instead of an open-ended one), verified by bringing the whole stack up under the new limits and confirming all four report healthy, TheHive's own web UI returns a real `200`, and none of the containers got OOM-killed (`docker inspect --format '{{.State.OOMKilled}}'` on each, all `false`). Cassandra and Cortex both settled close to their caps (Cassandra's JVM+page-cache accounting runs near 100% of a low limit by design, not a sign of failure), which is the real, current headroom — if this stack ever needs to run alongside more of the rest of the lab at once, these are the first numbers to revisit. This is still the heaviest, least-reused piece of the whole lab for what it proves (three verified case-creation flows), which is exactly why it stays stopped by default and only comes up on demand.
